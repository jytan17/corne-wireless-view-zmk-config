...
Arrows on M/N/E/I (vim HJKL feel). Page nav on bottom row. Media on right thumbs. Left home row = SK mod mirror. `MOU` at G-slot = hold to activate MOUSE layer. `BT⇧` at Z-slot = hold to activate BT layer (two-key gate prevents accidental profile switch).
...
## L6 — BT (hold NAV thumb + Z; right hand picks profile)

Nested layer, reached only from NAV. Hold NAV then hold Z to activate — release either to exit. Right home row picks profile 0-4. `CLR` on right-inner thumb wipes current profile pairing (both hands required = safe).
...
Profile order M N E I O = 0 1 2 3 4 (matches home row left-to-right).
